...
import { format, formatDistanceToNow, isPast, isToday, addDays as addD } from "date-fns";
import {
  ArrowLeft,
  CalendarDays,
  User,
...
export default function DealDetail() {
  ...
  const aiInsight = mockLeadSummaries.find(s => s.contact_id === deal.contact_id);

  // Build unified timeline
  const messages = mockMessages.filter(m => m.contact_id === deal.contact_id);
  const meetings = mockMeetings.filter(m => m.contact_id === deal.contact_id);
  const contactReminders = mockReminders.filter(r => r.contact_id === deal.contact_id && !r.is_done);
  ...
  const timeline: TimelineEntry[] = [
    ...
    ...(aiInsight
      ? [{ type: "ai_summary" as const, id: aiInsight.id, summary: aiInsight.summary, timestamp: aiInsight.generated_at }]
      : []),
    ...
    <div className="animate-fade-in">
      ...
      <div className="mt-3 mb-5">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${stageBadgeStyle[deal.stage]}`}>
            {stageLabel}
          </span>
        </div>
          ...
          <div className="mt-4 flex items-center gap-1 max-w-md">
            {progressStages.map((s, i) => (
              <div key={s.key} className="flex-1">
                ...
                <p className={`mt-1 text-[9px] text-center ${s.key === deal.stage ? "font-semibold text-primary" : "text-muted-foreground"}`}>
                  {s.label}
                </p>
              </div>
            ))}
      ...
      <div className="grid lg:grid-cols-[1fr_340px] gap-6">
        ...
        {/* RIGHT: Deal Card + AI Insights + Reminders */}
          ...
          <Card className="p-4">
            ...
            <div className="space-y-2.5">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${stageBadgeStyle[deal.stage]}`}>
                  {stageLabel}
                </span>
              </div>
              <div className="flex items-center justify-between">
...
function TimelineItem({ entry, isLast }: { entry: TimelineEntry; isLast: boolean }) {
  const iconBg = entry.type === "message"
    ? entry.channel === "whatsapp"
      ? "bg-[hsl(var(--channel-whatsapp))]/15"
      : entry.channel === "gmail"
        ? "bg-[hsl(var(--channel-gmail))]/15"
        : "bg-[hsl(var(--channel-calendar))]/15"
    : entry.type === "meeting"
      ? "bg-[hsl(var(--channel-calendar))]/15"
      : "bg-primary/10";
          ...
          <>
            <div className="flex items-center gap-2 mb-0.5">
              <span className="text-xs font-medium">
                {entry.direction === "outbound" ? "You" : entry.contactName}
              </span>
              <ChannelBadge channel={entry.channel} showLabel />
              {entry.direction === "outbound" && (
                <span className="rounded-full bg-muted px-1.5 py-0.5 text-[9px] text-muted-foreground">Sent</span>
              )}
            </div>
        ...
        {entry.type === "meeting" && (
          <>
            <p className="text-xs font-medium flex items-center gap-1">
              <CalendarDays className="h-3 w-3 text-[hsl(var(--channel-calendar))]" />
              Meeting: {entry.title}
            </p>
...
// ─── Reminder row with snooze ───────────────────────────────────
function ReminderRow({ reminder }: { reminder: typeof mockReminders[number] }) {
  ...
  const handleSnooze = (days: number, label: string) => {
    setSnoozed(true);
    setSnoozeLabel(label);
  };
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1.5 flex items-center gap-1">
            ...
            <Button variant="ghost" size="sm" className="h-5 px-1.5 text-[10px]" onClick={() => handleSnooze(1, "1 day")}>
              1d
            </Button>
            <Button variant="ghost" size="sm" className="h-5 px-1.5 text-[10px]" onClick={() => handleSnooze(3, "3 days")}>
              3d
            </Button>
            <Button variant="ghost" size="sm" className="h-5 px-1.5 text-[10px]" onClick={() => handleSnooze(7, "1 week")}>
              1w
            </Button>
          </div>
        </div>
