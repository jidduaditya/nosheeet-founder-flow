import { isToday, isPast, isFuture, format, formatDistanceToNow, subDays, differenceInDays } from "date-fns";
import { Card } from "@/components/ui/card";
import { mockReminders, mockDeals, DEAL_STAGES, type DealStage } from "@/lib/mock-data";
import {
  AlertCircle,
  Clock,
  CheckCircle2,
...
export default function Dashboard() {
  ...
  const dealsCreatedThisWeek = mockDeals.filter(d => new Date(d.created_at) >= oneWeekAgo).length;
  const dealsClosedThisWeek = mockDeals.filter(d => d.stage === "won" && new Date(d.updated_at) >= oneWeekAgo).length;
  const dealsGoingCold = mockDeals.filter(d =>
    !["won", "lost", "cold"].includes(d.stage) &&
    d.last_activity &&
    differenceInDays(new Date(), new Date(d.last_activity)) >= 7
  ).length;

  const pipelineStages = DEAL_STAGES;
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <SnapshotCard
          ...
          label="Going cold"
          value={dealsGoingCold}
          sub="no activity 7d+"
          accent="text-[hsl(var(--cold))]"
          warn={dealsGoingCold > 0}
        />
      </div>
            ...
            return (
              <div key={stage.key} className="min-w-[220px] flex-1">
                ...
                <div className="space-y-2">
                  {deals.map(d => {
                    const daysSinceActivity = d.last_activity
                      ? differenceInDays(new Date(), new Date(d.last_activity))
                      : 0;
